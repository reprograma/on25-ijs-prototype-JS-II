    ...
    static chamar(listaAnimais){
        ...
        for(let i = 0; i < listaAnimais.length; i++){
            ...
            if (animal instanceof Cachorro || (animal instanceof Gato && animal.social == true)) {
                animal.brincar();
                listaAnimaisResponderam.push(animal);
            }
        }

        return [listaAnimaisResponderam];
    }
}
...
class Cachorro extends AnimalDomestico{
    ...
    brincar(){
        if(this.ferido == true){
            console.log(`${this.nome} não consegue brincar pois está doente e escondido => esconder = ${this.#esconder}`)
        } else {
            super.brincar()
        }
    }
...
let listaAnimais = [mingau, zeus, hamtaro, jose];
AnimalDomestico.chamar(listaAnimais)